...
class BriefingIntelligence:
    ...
    def filter_papers_by_relevance(
        ...
        prompt = RELEVANCE_FILTER_PROMPT.format(
            profile_str=profile_str, papers_block=papers_block,
        )
    ...
    def generate_dynamic_queries(
        ...
        prompt = DYNAMIC_QUERIES_PROMPT.format(
            context_str=context_str, static_queries_str=static_queries_str,
        )
    ...
    def expand_topics(self, topics: List[str]) -> List[str]:
        ...
        prompt = TOPIC_EXPANSION_PROMPT.format(topic_list=topic_list)
    ...
    def summarize_papers(
        ...
        papers_block = "\n\n".join(paper_texts)
        prompt = PAPER_SUMMARIZATION_PROMPT.format(papers_block=papers_block)
    ...
    def score_papers_semantically(
        ...
        prompt = SEMANTIC_SCORING_PROMPT.format(
            interests_str=", ".join(topics), papers_block=papers_block,
        )
    ...
    def assess_reproduction_feasibility(
        ...
        papers_block = "\n\n".join(paper_texts)
        prompt = REPRODUCTION_ASSESSMENT_PROMPT.format(papers_block=papers_block)
    ...
    def rank_and_summarize_news(
        ...
        articles_block = "\n".join(news_lines)
        prompt = NEWS_RANKING_PROMPT.format(
            interests_str=", ".join(topics[:5]), articles_block=articles_block,
        )
        ...
        retry_result = self.bedrock.invoke(
            NEWS_RANKING_RETRY_PROMPT.format(articles_block=articles_block),
    ...
    def rank_and_summarize_blogs(
        ...
        blogs_block = "\n".join(blog_lines)
        prompt = BLOG_RANKING_PROMPT.format(
            interests_str=", ".join(topics[:5]), blogs_block=blogs_block,
        )
    ...
    def correlate_stocks_and_news(
        ...
        stocks_block = "\n".join(stock_lines)
        headlines_block = "\n".join(news_lines)
        prompt = STOCK_CORRELATION_PROMPT.format(
            stocks_block=stocks_block, headlines_block=headlines_block,
        )
    ...
    def detect_emerging_themes(
        ...
        prompt = EMERGING_THEMES_PROMPT.format(
            topics_str=topics_str, titles_block=titles_block,
        )
    ...
    def synthesize_briefing(
        ...
        prompt = SYNTHESIS_PROMPT.format(
            all_data=all_data, cross_source_note=cross_source_note,
        )
    ...
    def track_trending(
        ...
        prompt = TRENDING_TRACKING_PROMPT.format(
            today=today, items_block=items_block, trending_block=trending_block,
        )
    ...
    def generate_weekly_deep_dive(
        ...
        prompt = WEEKLY_DEEP_DIVE_PROMPT.format(context_str=context_str)
