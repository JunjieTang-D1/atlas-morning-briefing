...
<div class="footer">
  ...
  <a href="https://github.com/your-org/atlas-morning-briefing">GitHub</a>
    ...
    def send_podcast_email(
        ...
        """Send podcast MP3 as an email attachment.

        Args:
            recipients: List of email addresses.
            audio_bytes: MP3 audio data.
            filename: Attachment filename (e.g. "Atlas-Briefing-2026-04-06.mp3").
            date: Briefing date for the subject line.
        ...
        if not recipients:
            logger.warning("No email recipients configured for podcast")
            return {}

        date_str = date.strftime("%Y-%m-%d") if date else filename
        subject = f"Atlas Briefing Podcast — {date_str}"
            ...
            with self._connect_smtp() as server:
                for recipient in recipients:
                    try:
                        ...
                        msg.attach(MIMEText(
                            f"Atlas Morning Briefing Audio Overview for {date_str}.\n"
                            "Generated by NotebookLM.",
                            "plain",
                        ))

                        attachment = MIMEAudio(audio_bytes, _subtype="mpeg")
                        attachment.add_header(
                        ...
                        server.send_message(msg)
                        masked = (
                            recipient[:3] + "***" + recipient[recipient.index("@"):]
                            if "@" in recipient else "***"
                        )
                        logger.info(f"Podcast email sent to: {masked}")
                        results[recipient] = True
                        ...
                        logger.error(f"Failed to send podcast to {masked}: {e}")
                        results[recipient] = False

        except Exception as e:
            logger.error(f"SMTP connection failed for podcast email: {e}")
            for r in recipients:
                if r not in results:
                    results[r] = False
