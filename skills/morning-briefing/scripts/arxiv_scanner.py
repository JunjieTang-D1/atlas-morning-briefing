...
class DeepXivScanner:
    ...
    def search_topic(self, topic: str) -> list[dict[str, Any]]:
        ...
        try:
            ...
            # DeepXiv returns {"status": ..., "total_count": N, "result": [...]}
            # (older format used "results" and "total")
            if isinstance(response, dict):
                if "result" in response:
                    ...
                    logger.warning(f"Unexpected response keys: {list(response.keys())}")
                    raw_papers = []
            ...
            for r in raw_papers:
                ...
                if paper:
                    # Client-side date filtering (server-side date_from can be unreliable)
...
class ArxivScanner:
        ...
        try:
            ...
            for attempt in range(3):
                ...
                if response.status_code == 429:
                    ...
                    logger.warning(f"ArXiv 429 rate limit, waiting {wait}s (attempt {attempt+1}/3)")
                ...
                logger.error(f"ArXiv 429 after 3 retries for: {topic}")
                return []
    ...
    def scan_all_topics(self) -> list[dict[str, Any]]:
        """Scan all topics, deduplicate. Respects ArXiv rate limits (3s between requests)."""
        import time as _time
        all_papers = []
        seen_ids: set[str] = set()
        ...
        for i, topic in enumerate(self.topics):
            if i > 0:
                _time.sleep(3)  # ArXiv requires 3s between requests
            papers = self.search_topic(topic)
            for paper in papers:
...
def create_scanner(topics: list[str], days_back: int = 7, max_results: int = 20):
    """Create scanner. Always use legacy ArXiv API for freshness (real-time index).
    DeepXiv is 2-3 weeks behind on indexing, so only use for enrichment."""
    return ArxivScanner(topics=topics, days_back=days_back, max_results=max_results)


# Keep ArxivScanner as the primary class (real-time index)
# DeepXivScanner available for enrichment when needed
